...
For this project, we were interested to see if there was a Glass Ceiling at UVA-- specifically if there was a significant difference between male and female  salaries at UVA. In order to explore that, we explored the questions:
...
2. Can we predict an employee's gender based on their salary? What about if we add in tenure and/or department?

We decided to use a Decision Tree to explore which of these variables would be most important in determining gender. 

3. Can we predict an employee's tenure based on their gender? What about if we add in department and/or salary?

We decided to use a Decision Tree to explore which of these variables would be most important in determining tenure 

3. Can we predict the tenure based on salary (as well as other variables)?
...
#### Average Difference in Base Pay for Gender Between all UVA schools
```{r, echo=FALSE}
uva_data <- read.csv("All_UVA.csv")

#cleaning
uva_data <-na.omit(uva_data)
uva_data <- subset(uva_data , Gender %in% c("F", "M"))
uva_data$Gender <- as.factor(uva_data$Gender)
uva_data$Base.Pay <- as.numeric(gsub("[\\$,]", "", uva_data$Base.Pay))
uva_data <-uva_data[,c(4,5,8)] 
uva_data<-uva_data %>% filter(
  School == "School of Medicine"  | School == "School of Engineering and Applied Science" | School == "College of Arts and Sciences" | School == "McIntire School of Commerce" | School == "School of Engineering and Applied Science" | School == "Frank Batten School of Leadership and Public Policy" | School == "School of Law" | School == "Darden School of Business" | School == "School of Nursing" | School == "Curry School of Education"
  )
```

Across departments
#### Average Difference in Salary for Gender Across All SEAS Departments
```{r echo = FALSE}
data_diff <-data
data_diff$Salary <- as.numeric(gsub("[\\$,]", "", data_diff$Salary))
ggplot(data_diff, aes(fill=Gender, y=Salary, x=Department)) + 
...
We just simply had to run the function cor(), to discover how each column was related to each other to. We want to check if there are any highly correlated variables and remove them so as to not skew our KNN. We used a threshold of 0.7 to determine if something was too highly correlated.
...
training_tree <- data_tree[x,]
testing_tree <- data_tree[-x,]
```
...
If the employee is a member of the Computer Science or Biomedial, Chemical, Computer, Systems, Mechanical Engineering, or Materials Science department, the decision tree will then look at their position. If they are an Assistant Professor, then we will look at their department and if they are Systems Eng, Mechanical Eng, or Materials Science, then they are determined to be male.

Otherwise, we look at their salary, If it is above 135,000 then they are determined to be male, otherwise they are female.

If the employee is in the E+S (APMA) Department, the only important factor next to look at is their salary level. If it is below 77,000, then they are a male, if it above then they are female.

#### Variable Importance

```{r, echo=FALSE}
head(data_gini$variable.importance) 
```
...
```
The detection rate is found to be 66.7% with a prevalence of 90.5% Our model currently has an accuracy of 69.1%.
...
# Split your data into test and train using the caret
set.seed(2702)
y <- createDataPartition(data_tree$Gender,times=1,p = 0.8,list=FALSE)
training_tree2 <- data_tree[y,]
testing_tree2 <- data_tree[-y,]
```

```{r, echo=FALSE, include=FALSE, warning=FALSE}
data_long2 = data_tree %>% gather(Var, #<- list of predictor variables
                                Value,#<- the values of those predictor variables
                                -Tenure) 
data_long_form2 = ddply(data_long2, 
                            .(Var, Value),#<- group by Var and Value, "." 
                            #allows us to call the variables without quoting
                            summarize,  
                            prob_AGF = mean(as.numeric(ifelse(data$Tenure == "AGF", 1,0))),
                            prob_OT = mean(as.numeric(ifelse(data$Tenure == "On Track", 1,0))),
                            prob_T = mean(as.numeric(ifelse(data$Tenure == "Tenured", 1,0)))) #<- probability of not being Parent
```
...
data_gini2 = rpart(Tenure~.,  #<- formula, response variable ~ predictors
                           #   "." means "use all other variables in data"
                            method = "class",#<- specify method, use "class" for tree
                            parms = list(split = "gini"),#<- method for choosing tree split
                            data = training_tree2,#<- data used
                            control = rpart.control(cp=.01))
```
...
```{r, echo=FALSE}
rpart.plot(data_gini2, type =5, extra = 101)
```
...
```{r, echo=FALSE}
head(data_gini2$variable.importance) 
```
...
```{r, echo=FALSE}
tree_predict2 = predict(data_gini2,testing_tree, type= "class")
```
...
```{r, echo=FALSE, include=FALSE, warning=FALSE}
par_conf_matrix2<-confusionMatrix(as.factor(tree_predict2), as.factor(testing_tree2$Tenure), dnn=c("Prediction", "Actual"), mode = "sens_spec")
par_conf_matrix2
```
...
```{r, echo=FALSE, include=FALSE, warning=FALSE}
par_error_rate2 = 1-((par_conf_matrix2$table[1,1]+par_conf_matrix2$table[2,2]+par_conf_matrix2$table[3,3])/ nrow(testing_tree2))

...
For that purposes we decide to use KNN model and explore this question further. But as we will explain later, we did not have many answers to that question, so we were determined, and changed it. Or follow up questions were:
...
#### Average Difference in Base Pay for Gender
Between all UVA schools
```{r, echo=FALSE}
...
```{r}
#plotting
ggplot(uva_data, aes(School, Base.Pay))+ geom_bar(stat = "identity", aes(fill = Gender), position = "dodge")+ theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
Before entering this process, it is important to note that the KNN approach was the first method we wanted to use to attempt a ML model. However, it does not work for this data even if we are trying to predict the salary of the professors because . 

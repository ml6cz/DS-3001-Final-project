...
#data$Gender <- as.numeric(as.character(data$Gender))
data_tree$Gender[data_tree$Gender == "M"] <- 0
data_tree$Gender[data_tree$Gender == "F"] <- 1
...
x <- createDataPartition(data_tree$GenderNumeric,times=1,p = 0.8,list=FALSE)
training_tree <- data_tree[x, -5]
testing_tree <- data_tree[-x, -5]
```
...
data_long_form = ddply(data_long, 
                            ...
                            summarize,  
                            prob_Gender = mean(GenderNumeric), #<- probability of being Parent
                            prob_not_Gender = 1 - mean(GenderNumeric)) #<- probability of not being Parent
```

...
![Graph of Average Salary in SEAS by Factor](/cloud/project/All Factors.png)
...
data_gender <- data[]
data_factors <-data
data_factors$Salary <- as.numeric(gsub("[\\$,]", "", data_factors$Salary))


ggplot(data_factors, aes(fill=Gender, y=Salary, x=Department)) + 
...
#### Average Salary in SEAS by Factor
>>>>>>> 9a9072780172829099ebce57e71b12c136d02a74
...
```{r echo=FALSE, message = FALSE}
<<<<<<< HEAD
data_tree<- read.csv("SEAS_cleandata.csv")
=======
...
>>>>>>> 9a9072780172829099ebce57e71b12c136d02a74
vars <- c('Department', 'Position', 'Tenure','Gender', 'Salary')
data_tree <- data_tree[,(names(data_tree) %in% vars)]
data_tree <- subset(data_tree , Gender %in% c("F", "M"))
data_tree$Salary <- as.numeric(gsub("[\\$,]", "", data_tree$Salary))
data_tree$Gender <- as.factor(data_tree$Gender)
levels(data_tree$Gender)<- c(0,1)
data_tree$Gender <- as.numeric(as.character(data_tree$Gender))
```
